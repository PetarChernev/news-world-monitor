...
SERVICE_NAME="${SERVICE_NAME:-news-processor}"
IMAGE="gcr.io/${PROJECT_ID}/${SERVICE_NAME}"
SA_NAME="${SA_NAME:-sa-${SERVICE_NAME}}"
SA_EMAIL="${SA_EMAIL:-${SA_NAME}@${PROJECT_ID}.iam.gserviceaccount.com}"

# Optional env for the service
AI_API="${AI_API:-https://ai-api-1010480476071.europe-central2.run.app}"  # adjust or remove
...
ensure_api secretmanager.googleapis.com   # optional, safe to enable
...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --max-instances 50 \
  --concurrency 4 \
  --set-env-vars "PROJECT_ID=${PROJECT_ID},AI_API=${AI_API}"
